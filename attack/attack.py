...
class FGSMAttack(nn.Module):
    ...
    def perturb(self):
        ...
        loss = F.mse_loss(outputs, self.model(self.labels))
...
class PGDAttack(nn.Module):
    ...
    def perturb(self):
        ...
        criteria = RBSimCLRLoss(self.temperature, self. batch_size)
        with torch.enable_grad():
            for _iter in range(self.max_iters):
                ...
                if self.loss_type == 'mse':
                    loss = F.mse_loss(self.model(x), self.model(self.target))
                elif self.loss_type == 'sim':
                    #inputs = torch.cat((x, self.target))
                    #output = self.projector(self.model(inputs))
                    loss = criteria.forward(x, self.ori)


                elif self.loss_type == 'l1':
                    loss = F.l1_loss(self.model(x), self.model(self.target))

                elif self.loss_type == 'cos':
                    loss = 1 - F.cosine_similarity(self.model(x),
                                                   self.model(self.target)).mean()
        ...
        if self.loss_type == 'mse':
            loss = F.mse_loss(self.model(x), self.model(self.target)) * (1.0 / batch_size)
        elif self.loss_type == 'sim':
            ...
            loss = F.l1_loss(self.model(x), self.model(self.target)) * (1.0 / batch_size)
        elif self.loss_type == 'l2':
            loss = F.l2_loss(self.model(x), self.model(self.target))* (1.0 / batch_size)
        elif self.loss_type == 'cos':
            loss = 1 - F.cosine_similarity(self.model(x), self.model(self.target)).sum() * (
                        1.0 / batch_size)
